    ...
    <Section id="about" className="section-y border-t border-white/10">
      <div className="mx-auto grid max-w-7xl gap-12 lg:grid-cols-[0.9fr_1.1fr] lg:gap-20">
        <Reveal className="relative">
          <p className="mb-5 font-display text-xs font-semibold uppercase text-signal-cyan">
            About
          </p>
          ...
          <div className="space-y-7">
            {aboutCopy.map((paragraph) => (
              <motion.p
                key={paragraph}
                variants={fadeUp}
                className="text-xl leading-9 text-white/75 sm:text-2xl sm:leading-10"
              >
                {paragraph}
              </motion.p>
            ))}
          ...
          >
            {aboutHighlights.map((item, index) => (
              <div key={item} className="highlight-fragment">
                <span className="font-mono text-xs text-signal-cyan/80">
                  0{index + 1}
                </span>
                <p>{item}</p>
              </div>
